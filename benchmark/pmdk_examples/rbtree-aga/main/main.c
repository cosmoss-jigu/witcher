...
static TOID(struct tree_map) map;
...
void run_op(char **op, FILE *output_file) {
  ...
  switch (op[0][0]) {
    case 'i':
      ...
      witcher_tx_begin();
      ret = tree_map_insert(pop, map, key, value);
      witcher_tx_end();
    ...
    case 'd':
      ...
      witcher_tx_begin();
      value = tree_map_remove(pop, map, key);
      witcher_tx_end();
