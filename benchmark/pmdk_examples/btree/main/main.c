...
static TOID(struct btree_map) map;
...
void run_op(char **op, FILE *output_file) {
  ...
  switch (op[0][0]) {
    case 'i':
      ...
      witcher_tx_begin();
      ret = btree_map_insert(pop, map, key, value);
      witcher_tx_end();
    ...
    case 'd':
      ...
      witcher_tx_begin();
      value = btree_map_remove(pop, map, key);
      witcher_tx_end();
    ...
    case 'g':
      ...
      witcher_tx_begin();
      value = btree_map_get(pop, map, key);
      witcher_tx_end();
...
void init_map(char* pmem_path, size_t pmem_size_in_mib, char* layout_name) {
    ...
    root_obj* root_ptr = (root_obj*) pmemobj_direct(root);
	  btree_map_new(pop, &root_ptr->map, NULL);
		map = root_ptr->map;
	} else {
