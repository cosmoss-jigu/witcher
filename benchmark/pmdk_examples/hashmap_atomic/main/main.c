...
static TOID(struct hashmap_atomic) map;
...
void run_op(char **op, FILE *output_file) {
  ...
  switch (op[0][0]) {
    case 'i':
      ...
      witcher_tx_begin();
      ret = hm_atomic_insert(pop, map, key, value);
      witcher_tx_end();
    ...
    case 'd':
      ...
      witcher_tx_begin();
      value = hm_atomic_remove(pop, map, key);
      witcher_tx_end();
    ...
    case 'g':
      ...
      witcher_tx_begin();
      value = hm_atomic_get(pop, map, key);
      witcher_tx_end();
...
void init_map(char* pmem_path, size_t pmem_size_in_mib, char* layout_name) {
    ...
    witcher_tx_begin();
	  hm_atomic_new(pop, &root_ptr->map, NULL);
    witcher_tx_end();
		map = root_ptr->map;
	} else {
