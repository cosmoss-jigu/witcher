...
class WitcherParallelFullEngine3:
    ...
    def run(self):
        ...
        for tx_id in range(len(self.permutation_count_list)):
            total_permutation_count += self.permutation_count_list[tx_id]
        for tx_id in range(len(self.permutation_count_list)):
            permutation_count = self.permutation_count_list[tx_id]
            plan_count = permutation_count / total_permutation_count * TOTAL
            plan_count = int(plan_count + 0.5)
            if plan_count > 0:
                self.plan[tx_id] = plan_count
            elif permutation_count > TOTAL:
                self.plan[tx_id] = TOTAL
            elif permutation_count > 0:
                self.plan[tx_id] = permutation_count

        total_plan_count = 0
        with open('plan.txt', 'w') as f:
            for tx_id in self.plan.keys():
                plan_count = self.plan[tx_id]
                f.write(str(tx_id) + ':' + str(plan_count) + '\n')
                total_plan_count += plan_count
            f.write('TOTAL:' + str(total_plan_count) + '\n')
