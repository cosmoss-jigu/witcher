...
enum class RecordType : unsigned {
  ...
  FLType  = 'H',  // Cacheline Flush record
  FEType  = 'F',  // Memory Fence record
  TXADDType  = 'T',   // PMDK TX Add record
  TXALLOCType  = 'A',  // PMDK TX Alloc record
  MmapType  = 'M',  // mmap record
...
struct Entry {
  ...
  /// it belongs to.
  /// For Cacheline FLush Entries, it is the start address of the cachecline to
  /// be flushed.
  /// For Memory Fence entries, it is 0.
  uintptr_t address;
  ...
  /// For Cache Line and Memory Fence entries, it is 0.
  /// For Call: 1->withcer_tx_begin, 2->witcher_tx_end, 3->others
  uintptr_t length;
  ...
  /// A nice one-line method for initializing the structure with only id and tid
  explicit Entry(RecordType type, unsigned id, pthread_t tid) :
    type(type), id(id), tid(tid), address(0), length(0) {
  }
