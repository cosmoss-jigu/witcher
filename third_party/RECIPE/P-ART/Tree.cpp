...
#endif
void treeLoadKey(TID tid, Key &key) {
    return ;
}
namespace ART_ROWEX {
    ...
    void Tree::recover() {
      switch (root->getType()) {
          case NTypes::N4: {
              auto n = static_cast<N4 *>(root);
              n->recover();
              break;
          }
          case NTypes::N16: {
              auto n = static_cast<N16 *>(root);
              n->recover();
              break;
          }
          case NTypes::N48: {
              auto n = static_cast<N48 *>(root);
              n->recover();
              break;
          }
          case NTypes::N256: {
              auto n = static_cast<N256 *>(root);
              n->recover();
              break;
          }
      }
                       ...
                       char *layout_name)
    {
	 ...
	 root_obj *root_obj = init_nvmm_pool(path, size, layout_name, &is_created);
   	 if (is_created == 0) {
	      printf("Reading from an existing p-art.\n");
        Tree* tree = (Tree*)root_obj->p_art_ptr;
        tree->recover();
 	      return tree;
    	}
    	Tree *tree= new Tree(treeLoadKey);

    	root_obj->p_art_ptr = tree;
	N::clflush((char *)root_obj, sizeof(root_obj),true,true);
    	return tree;
    }
	...
	std::cout << "Searching key " << k->fkey << std::endl;
        while (true) {
