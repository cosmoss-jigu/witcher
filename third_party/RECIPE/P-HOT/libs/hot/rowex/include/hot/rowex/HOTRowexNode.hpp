...
template<typename DiscriminativeBitsRepresentation, typename PartialKeyType>  void* HOTRowexNode<DiscriminativeBitsRepresentation, PartialKeyType>::operator new (size_t , uint16_t const numberEntries) {
	...
	int error = nvm_aligned_alloc(&memoryForNode, SIMD_COB_TRIE_NODE_ALIGNMENT, allocationInformation.mTotalSizeInBytes);
	if(error != 0) {
		//"Got error on alignment"
		printf("[%s:%d] NVM-Alloc-Fault\n", __func__, __LINE__);
		throw std::bad_alloc();
	}
...
*/
/*
template<> void HOTRowexNode<hot::commons::SingleMaskPartialKeyMapping, unsigned char>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}

template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<hot::commons::MultiMaskPartialKeyMapping<4u>, unsigned int>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}


template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<hot::commons::SingleMaskPartialKeyMapping, unsigned short>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}

template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<hot::commons::SingleMaskPartialKeyMapping, unsigned int>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}

template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<>hot::commons::MultiMaskPartialKeyMapping<1u>, unsigned char::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}

template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<hot::commons::MultiMaskPartialKeyMapping<1u>, unsigned short>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}

template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<hot::commons::MultiMaskPartialKeyMapping<1u>, unsigned int>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}

template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> void HOTRowexNode<hot::commons::MultiMaskPartialKeyMapping<2u>, unsigned short>::operator delete (void * rawMemory) {
	//free(rawMemory);
	nvm_free(rawMemory);
}*/
...
template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> template<typename SourceDiscriminativeBitsRepresentation, typename SourcePartialKeyType> inline HOTRowexNode<DiscriminativeBitsRepresentation, PartialKeyType>::HOTRowexNode(
	...
	HOTRowexChildPointer const * existingPointers = sourceNode.getPointers();
	HOTRowexChildPointer * targetPointers = this->getPointers();

	SourcePartialKeyType const * existingMasks = sourceNode.mPartialKeys.mEntries;
	PartialKeyType * targetMasks = mPartialKeys.mEntries;
...
template<typename DiscriminativeBitsRepresentation, typename PartialKeyType> template<typename SourceDiscriminativeBitsRepresentation, typename SourcePartialKeyType> inline HOTRowexNode<DiscriminativeBitsRepresentation, PartialKeyType>::HOTRowexNode(
	...
	HOTRowexChildPointer const * sourcePointers = sourceNode.getPointers();
	HOTRowexChildPointer* targetPointers = this->getPointers();

	SourcePartialKeyType const * sourceMasks = sourceNode.mPartialKeys.mEntries;
	PartialKeyType * targetMasks = mPartialKeys.mEntries;
