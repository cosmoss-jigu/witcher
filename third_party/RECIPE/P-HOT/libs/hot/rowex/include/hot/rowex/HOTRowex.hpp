...
/*	EpochBasedMemoryReclamationStrategy memReclamation;
	MemoryGuard memoryGuard(&memReclamation);*/
...
template<typename ValueType, template <typename> typename KeyExtractor>
inline void HOTRowex<ValueType, KeyExtractor>::recovery(){
	//mRoot.unlock();
	static EpochBasedMemoryReclamationStrategy memReclamation;
        mMemoryReclamation = EpochBasedMemoryReclamationStrategy::getInstance();
	mRoot.recoverSubtree();
}
