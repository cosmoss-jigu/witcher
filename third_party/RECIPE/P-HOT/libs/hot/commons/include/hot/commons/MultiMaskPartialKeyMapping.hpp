...
// Add new function to solve the compilation error
template<unsigned int numberExtractionMasks> template<typename Operation> inline auto MultiMaskPartialKeyMapping<numberExtractionMasks>::executeWithCorrectMaskAndDiscriminativeBitsRepresentation(Operation const & operation) const {
	assert(getMaximumMaskByteIndex(calculateNumberBitsUsed()) <= 3);
	if(numberExtractionMasks==1u){
	...
	else if(numberExtractionMasks==2u){
	...
	else if(numberExtractionMasks==4u){
		return operation(*this, static_cast<uint32_t>(UINT32_MAX));
	}
...
template<unsigned int numberExtractionMasks> inline typename MultiMaskPartialKeyMapping<numberExtractionMasks>::ExtractionDataArray MultiMaskPartialKeyMapping<numberExtractionMasks>::mapInput(uint8_t const * keyBytes) const {
