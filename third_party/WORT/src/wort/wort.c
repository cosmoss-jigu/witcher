                       ...
                       char *layout_name)
{
    ...
    if (is_created == 0) {
      printf("Reading from an existing wort.\n");
      return (art_tree*)root_obj->wort_ptr;
    }
...
static art_leaf* make_leaf(const unsigned long key, int key_len, void *value, bool flush) {
	...
	art_leaf *l;
    void *ret;
	nvm_aligned_alloc(&ret, 64, sizeof(art_leaf));
    l = ret;
	l->value = value;
	l->key_len = key_len;
	l->key = key;
