...
static art_node* alloc_node(uint8_t type) {
	...
	switch (type) {
		case NODE4:
			...
			//nvm_aligned_alloc(&ret, 64, sizeof(art_node4));
      ret = nvm_alloc(sizeof(art_node4));
            n = ret;
			for (i = 0; i < 4; i++)
		...
		case NODE16:
			...
			//nvm_aligned_alloc(&ret, 64, sizeof(art_node16));
      ret = nvm_alloc(sizeof(art_node16));
            n = ret;
			((art_node16 *)n)->bitmap = 0;
			break;
		case NODE48:
		//	posix_memalign(&ret, 64, sizeof(art_node48));
			//nvm_aligned_alloc(&ret, 64, sizeof(art_node48));
      ret = nvm_alloc(sizeof(art_node48));
            n = ret;
			memset(n, 0, sizeof(art_node48));
			break;
		case NODE256:
		//	posix_memalign(&ret, 64, sizeof(art_node256));
			//nvm_aligned_alloc(&ret, 64, sizeof(art_node256));
      ret = nvm_alloc(sizeof(art_node256));
            n = ret;
...
static art_leaf* make_leaf(const unsigned long key, int key_len, void *value, bool flush) {
	...
	art_leaf *l;
//	posix_memalign(&l, 64, sizeof(art_leaf));
	// nvm_aligned_alloc(&l, 64, sizeof(art_leaf));
  l = nvm_alloc(sizeof(art_leaf));
	l->value = value;
	l->key_len = key_len;
	l->key = key;
                       ...
                       char *layout_name)
{
    ...
    if (is_created == 0) {
      printf("Reading from an existing woart.\n");
      return (art_tree*)root_obj->woart_ptr;
    }
    art_tree *tree= nvm_alloc(sizeof(art_tree));
    art_tree_init(tree);
    flush_buffer(tree, sizeof(art_tree), true);

    root_obj->woart_ptr = tree;
    flush_buffer((char *)root_obj, sizeof(root_obj),true);
    return tree;
}
