...
char *btree::btree_search(entry_key_t key){
  ...
  // the condition: "(char *)t != (char *)key" is for the specific test case
  if(!t) {
...
void btree::btree_delete(entry_key_t key) {
  ...
  if(p) {
    if(!p->remove(this, key)) {
      // TODO may have stack overflow here
      // btree_delete(key);
                       ...
                       char *layout_name)
{
    ...
    if (is_created == 0) {
      printf("Reading from an existing fast-fair.\n");
      return (btree*)root_obj->fnf_ptr;
    }
    btree *bt= new btree();
