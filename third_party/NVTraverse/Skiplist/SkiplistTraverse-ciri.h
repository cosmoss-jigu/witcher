...
template <class T> class SkiplistTraverse {
  ...
  //          // ssfree(set);
    ...
    newNode = (Node*) nvm_alloc(sizeof(Node));
...
SkiplistTraverse<int> *init_skiplist(char *path,
                              size_t size,
                              char *layout_name)
{
    int is_created;
    ...
    if (is_created == 0) {
      ...
      SkiplistTraverse<int>* ret = (SkiplistTraverse<int>*) root_obj->ptr;
      return ret;
    }
    SkiplistTraverse<int>* list = new SkiplistTraverse<int>();
