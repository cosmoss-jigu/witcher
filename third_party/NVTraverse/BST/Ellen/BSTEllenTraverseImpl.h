...
#ifndef ELLEN_TRAVERSE_IMPL_H
#define ELLEN_TRAVERSE_IMPL_H
#include "BSTEllenTraverse.h"
...
sval_t BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_find(const int tid, skey_t key/*, EpochThread epoch*/) {
        ...
        // end traversal
        if(l->original_parent) FLUSH_node(tid, l->original_parent);
        //FLUSH_node(tid, l);      
...
template <typename skey_t, typename sval_t, class RecMgr>
sval_t BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_insert(const int tid, const skey_t key, const sval_t value/*, EpochThread epoch*/) {
        ...
        auto guard = recmgr->getGuard(tid);
            ...
            // end traversal
            if(p->original_parent) FLUSH_node(tid, p->original_parent);
            FLUSH_node(tid, p);
            //FLUSH_node(tid, l);
        ...
        if (GETFLAG(pupdate) != STATE_CLEAN) {
            ...
            new_node->original_parent = p;
            FLUSH_STRUCT(tid, new_node);

            auto new_sibling = create_node(tid, l->key, l->value, NULL, NULL/*, epoch*/);
            new_sibling->original_parent = p;
            FLUSH_STRUCT(tid, new_sibling);
            ...
            auto new_internal = (key < l->key)
                ...
                new_internal->original_parent = p;
            FLUSH_STRUCT(tid, new_internal);
		...
		//finalize_node_list_gen((void*)new_node, NULL, NULL);
                //finalize_node_list_gen((void*)new_sibling, NULL, NULL);
                //finalize_node_list_gen((void*)new_internal, NULL, NULL);
...
template <typename skey_t, typename sval_t, class RecMgr>
sval_t BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_delete(const int tid, skey_t key/*, EpochThread epoch*/) {
	...
	auto p = root;
            ...
            if(gp)
            {
                // gp->original_parent == NULL means gp is always reachable
                if(gp->original_parent) FLUSH_node(tid, gp->original_parent); 
                FLUSH_node(tid, gp);  
            }
            FLUSH_node(tid, p); 
            //FLUSH_node(tid, l);  
...
template <typename skey_t, typename sval_t, class RecMgr>
bool BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_cas_child(const int tid, node_t<skey_t, sval_t> * parent, node_t<skey_t, sval_t> * old, node_t<skey_t, sval_t> * nnode) {
...
template <typename skey_t, typename sval_t, class RecMgr>
void BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_help_insert(const int tid, info_t<skey_t, sval_t> * op/*, EpochThread epoch*/) {
...
template <typename skey_t, typename sval_t, class RecMgr>
bool BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_help_delete(const int tid, info_t<skey_t, sval_t> * op/*, EpochThread epoch*/) {
...
template <typename skey_t, typename sval_t, class RecMgr>
void BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_help_marked(const int tid, info_t<skey_t, sval_t> * op/*, EpochThread epoch*/) {
...
template <typename skey_t, typename sval_t, class RecMgr>
void BSTEllenTraverse<skey_t, sval_t, RecMgr>::bst_help(const int tid, info_t<skey_t, sval_t> * u/*, EpochThread epoch*/) {
