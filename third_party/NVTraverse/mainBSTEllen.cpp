...
__thread ssmem_allocator_t *allocI;
__thread void* nodes[1024];

#include "BST/Ellen/adapter.h"
...
void threadRun(thread_data *tData, bool nv) {
    ...
    allocI = (ssmem_allocator_t *)malloc(sizeof(ssmem_allocator_t));
    ssmem_alloc_init(allocI, SSMEM_DEFAULT_MEM_SIZE, id, nv);
    ...
    for (int i = 0; i < (uint32_t)elementsToInsert; i++) {
        ...
        if (!set->insertIfAbsent(id, key, id)) {
            i--;
        }
    }
...
static void run(bool nonVolatile) {
    ...
    SET* set = new SET(NUM_THREADS, INT_MIN,INT_MAX, NULL, NULL);
    if (ITERATION == 1) {
...
int main(int argc, char **argv) {
    ...
    if (!ALG_NAME.compare("BSTEllenOriginal")) {
    	run<ds_adapter<BSTEllenOriginal<int, int, record_manager<reclaimer_none<>, allocator_new<>,  pool_none<>, node_t<int,int>, info_t<int,int>> >, int, int, reclaimer_none<>, allocator_new<>, pool_none<> >>(false);
    } else if (!ALG_NAME.compare("BSTEllenIz")) {
        run<ds_adapter<BSTEllenIz<int, int, record_manager<reclaimer_none<>, allocator_new<>,  pool_none<>, node_t<int,int>, info_t<int,int>> >, int, int, reclaimer_none<>, allocator_new<>, pool_none<> > >(true);
    } else if (!ALG_NAME.compare("BSTEllenTraverse")) {
        run<ds_adapter<BSTEllenTraverse<int, int, record_manager<reclaimer_none<>, allocator_new<>,  pool_none<>, node_t<int,int>, info_t<int,int>> >, int, int, reclaimer_none<>, allocator_new<>, pool_none<> >>(true);
    } else {
