...
#include "Skiplist/SkiplistOriginal.h"
#include "Skiplist/SkiplistIz.h"
#include "Skiplist/SkiplistTraverse.h"
...
static void run(bool nonVolatile) {
    ...
    SET* set = new SET();
    levelmax = floor_log_2(KEY_RANGE);
    if (ITERATION == 1) {
...
int main(int argc, char **argv) {
    ...
    if (!ALG_NAME.compare("SkiplistOriginal")) {
            run<SkiplistOriginal<int>>(false);
    } else if (!ALG_NAME.compare("SkiplistIz")) {
            run<SkiplistIz<int>>(true);
    } else if (!ALG_NAME.compare("SkiplistTraverse")) {
            run<SkiplistTraverse<int>>(true);
    }
    else {
