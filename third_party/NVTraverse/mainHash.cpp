...
#include "Hash/HashOriginal.h"
#include "Hash/HashIz.h"
#include "Hash/HashTraverse.h"
...
static bool parseArgs(int argc, char **argv) {
    int arg;
    while ((arg = getopt(argc, argv, "A:T:D:R:K:I:L:H")) != -1) {
        switch (arg) {
            case 'A':
...
static void run(bool nonVolatile) {
    ...
    uint32_t initialSize = (KEY_RANGE/2)/LOAD_FACTOR;	
    SET* set = new SET(initialSize);
    if (ITERATION == 1) {
        cout << "##" << ALG_NAME << " - Initial: " << KEY_RANGE / 2 << " / Range: " << KEY_RANGE;
        cout << " / Load: " << LOAD_FACTOR << " / Update: " << 100 - R_RATIO;
...
int main(int argc, char **argv) {
    ...
    if (!ALG_NAME.compare("HashOriginal")) {
            run<HashOriginal<int>>(false);
    } else if (!ALG_NAME.compare("HashIz")) {
            run<HashIz<int>>(true);
    } else if (!ALG_NAME.compare("HashTraverse")) {
            run<HashTraverse<int>>(true);
    }
    else {
